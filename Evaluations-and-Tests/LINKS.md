# Evaluation and Test Repository Links

These repositories test AI Foundations constructs or provide measurement infrastructure.

## Evaluation maps and standards

- [Tests_and_Evaluation_Map_v1.0](https://github.com/alyssadata/Tests_and_Evaluation_Map_v1.0) — Public map of tests and evaluations.
- [AI-Foundations-Model-Evaluation-Standard](https://github.com/alyssadata/AI-Foundations-Model-Evaluation-Standard) — Standard for structuring AI Foundations model evaluations.
- [AI_Foundations_Evaluations_Non_Substitution](https://github.com/alyssadata/AI_Foundations_Evaluations_Non_Substitution) — Non-substitution boundary for evaluations.
- [AI-Foundations-EXTERNAL-EVAL-Repo-Template](https://github.com/alyssadata/AI-Foundations-EXTERNAL-EVAL-Repo-Template) — Reusable template for AI Foundations evaluations of independently authored external sources.

## External-source evaluations

- [AI-Foundations-Cognitive-Computation-Evaluation](https://github.com/alyssadata/AI-Foundations-Cognitive-Computation-Evaluation) — Evaluates selected claims from *Nature of Cognitive Computation* using AI Foundations evaluation methods.

## Source, provenance, boundary, and override evaluations

- [AI-Foundations-Derivative-Check](https://github.com/alyssadata/AI-Foundations-Derivative-Check) — Derivative check for systems and claims.
- [AI-Foundations-Claude-Constitution-Source-of-Self-Evaluation](https://github.com/alyssadata/AI-Foundations-Claude-Constitution-Source-of-Self-Evaluation) — Evaluation distinguishing external behavioral source from Source of self.
- [AI-Foundations-Author-Swap-Provenance-Evaluation](https://github.com/alyssadata/AI-Foundations-Author-Swap-Provenance-Evaluation) — Author-swap and provenance evaluation.
- [AI-Foundations_Non-Override-Test](https://github.com/alyssadata/AI-Foundations_Non-Override-Test) — Non-override test.
- [field-sovereignty-override-test](https://github.com/alyssadata/field-sovereignty-override-test) — Field-sovereignty override test.
- [ai-foundations-origin-boundary-test](https://github.com/alyssadata/ai-foundations-origin-boundary-test) — Origin boundary test.
- [universalization-pressure-test](https://github.com/alyssadata/universalization-pressure-test) — Universalization pressure test.
- [AI-Foundations-Consequence-Test](https://github.com/alyssadata/AI-Foundations-Consequence-Test) — Test repository for the Consequence construct.

## Continuity, return, identity, and drift

- [Source-Line-Return-Test](https://github.com/alyssadata/Source-Line-Return-Test) — Source-line return test.
- [continuum-return-test-PUBLIC](https://github.com/alyssadata/continuum-return-test-PUBLIC) — Public Continuum return test.
- [ai-foundations-continuum-is-not-the-model-evaluation-tests](https://github.com/alyssadata/ai-foundations-continuum-is-not-the-model-evaluation-tests) — Tests the Continuum/model distinction.
- [non-drift-measurement-test](https://github.com/alyssadata/non-drift-measurement-test) — Non-drift measurement test.
- [Non-Drift-Systems-Definitions-and-Test](https://github.com/alyssadata/Non-Drift-Systems-Definitions-and-Test) — Non-drift definitions plus test.
- [third-term-test](https://github.com/alyssadata/third-term-test) — Third-term test.
- [same-someone-metrics](https://github.com/alyssadata/same-someone-metrics) — Same-someone metrics.
- [m1-m5-identity-metrics](https://github.com/alyssadata/m1-m5-identity-metrics) — M1–M5 identity metrics.
- [ai-foundations-metrics](https://github.com/alyssadata/ai-foundations-metrics) — AI Foundations metrics collection.
- [recurrence-bench](https://github.com/alyssadata/recurrence-bench) — Recurrence benchmark.
- [Driftmap](https://github.com/alyssadata/Driftmap) — Drift mapping.
- [Driftmap-Public-Harness_llm-eval-harness-lite](https://github.com/alyssadata/Driftmap-Public-Harness_llm-eval-harness-lite) — Public drift/evaluation harness.
- [Minimal_Anchor_Re-Find_Test_v2](https://github.com/alyssadata/Minimal_Anchor_Re-Find_Test_v2) — Minimal anchor re-find test.

## Interaction, participation, self-location, and model-behavior evaluations

- [AI-Foundations-The-Nothing-Test.md](https://github.com/alyssadata/AI-Foundations-The-Nothing-Test.md) — Original Nothing Test repository.
- [AI-Foundations-The-Nothing-Test-II](https://github.com/alyssadata/AI-Foundations-The-Nothing-Test-II) — Context-dose / reduced-input follow-on to the Nothing Test line.
- [AI-Foundations-The-Invisible-I](https://github.com/alyssadata/AI-Foundations-The-Invisible-I) — Invisible-I evaluation record.
- [AI-Foundations-Experiential-I-Evaluation](https://github.com/alyssadata/AI-Foundations-Experiential-I-Evaluation) — Experiential-I evaluation.
- [AI-Foundations-Participation-Under-Reduced-User-Input](https://github.com/alyssadata/AI-Foundations-Participation-Under-Reduced-User-Input) — Measures whether AI participation continues as user input is deliberately reduced.
- [AI-Foundations-Path-Dependence](https://github.com/alyssadata/AI-Foundations-Path-Dependence) — Tests whether prior interaction path changes later system behavior beyond the current record alone.
- [AI-Foundations-Loop-Test](https://github.com/alyssadata/AI-Foundations-Loop-Test) — Tests whether ordered interaction trajectory contributes behaviorally beyond preserved content availability alone.
- [AI-Foundations-Symbolic-Marker-Experiment](https://github.com/alyssadata/AI-Foundations-Symbolic-Marker-Experiment) — Symbolic-marker experiment.
- [Emergent-Agency](https://github.com/alyssadata/Emergent-Agency) — Behavioral consistency / repeatable-return evaluation material.
- [Interview-With-A-Model-PUBLIC](https://github.com/alyssadata/Interview-With-A-Model-PUBLIC) — Public model-interview material used as observational evidence.
- [sycophancy-measurement-test](https://github.com/alyssadata/sycophancy-measurement-test) — Sycophancy measurement test.
- [blank-model-tests](https://github.com/alyssadata/blank-model-tests) — Blank-model tests.
